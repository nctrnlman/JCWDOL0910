...
function AddressCard() {
    ...
    const editAddress = async (id_address, event) => {
        ...
        try {
            ...
            const editedAddress = temp_address.find((address) => address.id_address === id_address);

            if (token) {
                let response = await axios.post(
                    `http://localhost:8000/user_profile/edit-address/${id_address}`,
                    editedAddress,
                    {
        ...
        try {
            ...
            if (token) {
                let response = await axios.delete(
                    `http://localhost:8000/user_profile/delete-address/${id_address}`,
                    {
                        headers: { Authorization: `Bearer ${token}` },
                    }
                );
    ...
    return (
        <div>
            <div className="px-4 sm:px-0">
                <h3 className="text-base font-semibold leading-7 text-gray-900">Addresses</h3>
                <p className="mt-1 max-w-2xl text-sm leading-6 text-gray-500">Personal addresses.</p>
            </div>
                            ...
                            <dl className="divide-y divide-gray-100">
                                <form id="address">
                                    <div className="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
                                        ...
                                        <input
                                            ...
                                            onChange={(e) => handleInputChange(address_point.id_address, e)}
                                        />
                                    </div>
                                    <div className="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
                                ...
                                {changed ? (
                                    <div className="grid gap-4 grid-cols-8">
                                        ...
                                        <button className="inline-flex items-center rounded-md bg-blue-50 px-2 py-1 text-m font-medium text-blue-700 ring-1 ring-inset ring-blue-700/10 text-center" onClick={(e) => editAddress(address_point.id_address, e)}>Save</button>

                                    </div>
                                )
                                    : null}
                                <button className="inline-flex items-center rounded-md bg-blue-50 px-2 py-1 text-m font-medium text-blue-700 ring-1 ring-inset ring-blue-700/10 text-center" onClick={(e) => deleteAddress(address_point.id_address, e)}><AiFillDelete /></button>
